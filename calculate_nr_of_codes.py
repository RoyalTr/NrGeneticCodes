...
Calculate the number of possible genetic codes.
    ...
    k = the number of amino acids,       (e.g. 21 = 20 amino acids + 1 stop)

So n is the number of "items" (codons) being assigned, and k is the number of
distinct "labels"

A genetic code is an assignment of every codon to one amino acid, using each
amino acid at least once. Because amino acids are distinct labels, the count is
the number of surjective (onto) functions from the n codons onto the k amino
acids:
...
Input : in_parameters.txt   (semicolon-separated: n ; k  ->  codons ; amino acids)
        ...
        default pair (64 codons ; 21 amino acids) shown above.
...
# 64 codons and 21 amino acids (20 + 1 stop).
DEFAULT_K = 21         # number of amino acids
DEFAULT_N = 64         # number of codons

# Title row expected as the first line of the input file (it is skipped when read).
INPUT_HEADER = SEP.join(["number of codons", "number of amino acids"])
...
def ensure_input_file() -> None:
    ...
    print(
        ...
        f"with the default values ({DEFAULT_N} codons ; {DEFAULT_K} amino acids)."
    )
...
def genetic_codes(n: int, k: int) -> int:
    """Exact number of genetic codes = k! * S(n, k)  (n = codons, k = amino acids)."""
...
def validate(k: int, n: int) -> str | None:
    ...
    if k <= 1:
        return f"number of amino acids ({k}) must be > 1"
    if n <= 1:
        return f"number of codons ({n}) must be > 1"
    if k > n:
        return (f"more amino acids ({k}) than codons ({n}) "
                "is impossible")
    return None
...
def main() -> None:
    ...
    header = SEP.join(
        ["number of codons", "number of amino acids", "possible number of codes"]
    )
    print(header)
